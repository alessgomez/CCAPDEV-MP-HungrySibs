...
function getBestSellers(resolve, reject){
    ...
    BestSeller.find().populate("productId").exec(function(err, results){
        ...
        for (var i=0;i < results.length; i++)
        {
            ...
            bestSellers.push(productObj);
        }
        resolve(bestSellers);
    });
}

function getAddOnContents(prodId, resolve, reject){

    
    Product.find({id: prodId}).populate("addOn").exec(function(err, results){
        if (err) 
        {
        ...
        {
            console.log('product find not error');
            console.log(results);
            var addOns = [];
            for (var i=0;i < results[0].addOn.length; i++)
            {
                var addOnObj = {
                    id: results[0].addOn[i].id,
                    name: results[0].addOn[i].name,
                    price: results[0].addOn[i].price
                };
...
const controller = {
    ...
    getIndex: function (req, res) {
        ...
        let q = new Promise((resolve, reject) => {
            return getBestSellers(resolve, reject);
        })
            ...
            db.findOne(Account, {userID: req.session.user}, {}, function(user) {
                if (user != null)
                {
                    Order.find({account: user._id}).lean().then((orderList) => {
                        ...
                        curr.subtotal = curr.subtotal.toFixed(2);
                        curr.deliveryFee = curr.deliveryFee.toFixed(2);

                        if (typeof curr.discount != "undefined")
                            curr.discount = curr.discount.toFixed(2);
                        
                        curr.total = curr.total.toFixed(2);
                        const data = {
                            style: ["navbar", "orderhistory"],
                        ...
                        let p = new Promise((resolve, reject) =>{
                            return getOrderContents(curr.orderId, resolve, reject);
                        })
                
                        p.then((order) => {
                            data.particulars = order;
                            res.render("orderhistory", data);
                        }).catch((message) => {
                            console.log("This is in catch: " + message);
                        })
    ...
    getAddToBag: function (req, res) {
        ...
        p.then((bag) => {
            ...
            db.findOne(Product, query, projection, function(result) {
                ...
                let q = new Promise((resolve, reject) =>{
                    return getAddOnContents(prodId, resolve, reject);
                })
